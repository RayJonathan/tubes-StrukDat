...
// BRYAN IMANUEL - 1121013
// TERRY LOUIS FUJINAMI - 1121015
...
public class Main {
    ...
    static void masuk() {
        ...
        if (queueIndex == -1) {
            ...
            if (slotIndex != -1) {
                ...
                JOptionPane.showMessageDialog(null, "Maaf restoran sedang Penuh!", "Restoran Status",
    ...
    static void menuKonfirmasi() {
        ...
        if (validate == JOptionPane.YES_OPTION) {
            Keranjang tempKeranjang = new Keranjang();
            for (int index = 0; index < clients[counter - 1].keranjang.jumlah[0].length; index++) {
                tempKeranjang.jumlah[0][index] = clients[counter - 1].keranjang.jumlah[0][index];
            }
            for (int index = 0; index < clients[counter - 1].keranjang.jumlah[1].length; index++) {
                tempKeranjang.jumlah[1][index] = clients[counter - 1].keranjang.jumlah[1][index];
            }
            admin.penghasilan += clients[counter - 1].keranjang.totalKeranjang;
            tempKeranjang.totalKeranjang = clients[counter - 1].keranjang.totalKeranjang;
            for (int i = 0; i < 4; i++) {
            ...
            histories.add(new History(clients[counter - 1].username, tempKeranjang));
        } else {
            for (int i = 0; i < 2; i++) {
                for (int j = 0; j < 4; j++) {
    ...
    static void menuKonfirmasi(int index) {
        ...
        if (validate == JOptionPane.YES_OPTION) {
            Keranjang tempKeranjang = new Keranjang();
            for (int i = 0; i < clients[index].keranjang.jumlah[0].length; i++) {
                tempKeranjang.jumlah[0][i] = clients[index].keranjang.jumlah[0][i];
            }
            for (int i = 0; i < clients[index].keranjang.jumlah[1].length; i++) {
                tempKeranjang.jumlah[1][i] = clients[index].keranjang.jumlah[1][i];
            }
            admin.penghasilan += clients[index].keranjang.totalKeranjang;
            tempKeranjang.totalKeranjang = clients[index].keranjang.totalKeranjang;
            for (int i = 0; i < 4; i++) {
                admin.total.penjualan[0][i] += clients[index].keranjang.jumlah[0][i];
                admin.total.penjualan[1][i] += clients[index].keranjang.jumlah[1][i];
            }
            histories.add(new History(clients[index].username, tempKeranjang));
        } else {
            for (int i = 0; i < 2; i++) {
                for (int j = 0; j < 4; j++) {
    ...
    static void waitingList(String name) {
        if (IsFull(queue)) {
            ...
            int ask = JOptionPane.showConfirmDialog(null, "Apakah anda ingin masuk antrian atau tidak");
            switch (ask) {
    ...
    static void deletePelanggan() {
        ...
        if (counts >= 5) {
            ...
            temp = clients[counts].username;
            clients[counts].username = "";
            for (int i = 0; i < 2; i++) {
                for (int j = 0; j < 4; j++) {
            ...
            counts++;
            JOptionPane.showMessageDialog(null,
                    "Yang di delete : " + temp + "\nDidalam restaurant tersisa : \n" + textDeletePelanggan());
        }
        menuAdmin();
    }

    static String textDeletePelanggan() {
        String text = "";
        for (int i = 0; i < clients.length; i++) {

            text += (i + 1) + " . " + clients[i].username + " \n";
        }
        return text;
    }

    static void historyCheck(String nama) {
        String text = "";
        int temp;

        for (int k = 0; k < histories.size(); k++) {
            if (histories.get(k).name.equals(nama)) {
                text += histories.get(k).name + "\n";
                for (int i = 0; i < 2; i++) {
                    for (int j = 0; j < 4; j++) {
                        temp = histories.get(k).keranjang.jumlah[i][j];
                        if (temp != 0) {
                            if (i == 0) {
                                text += makanan[i].nama + " sebanyak " + temp + "\n";
                            } else {
                                text += minuman[i].nama + " sebanyak " + temp + "\n";
                            }
                        }
                    }
                }
        ...
        JOptionPane.showMessageDialog(null, "HISTORY : " + text);
